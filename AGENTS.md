...
Public web UI for the **screeners vault** (`be1eza/screeners` — the Finviz→Obsidian pipeline).
Vite + React + TypeScript + MUI, deployed to GitHub Pages via Actions.

## The one rule

The **vault is the source of truth.** This app reads the public vault over HTTP and writes no
durable domain state — no API keys and no repository mutations. The planned private live refresh
may keep one disposable response in browser IndexedDB, but that cache is never historical truth;
deleting and rebuilding the app or clearing the browser loses nothing. See
`LIVE_REFRESH_ARCHITECTURE.md` for the cross-system boundary.
...
The vault's own `CLAUDE.md` + `AGENT.md + `repo.md` are authoritative; this is the reader's-eye summary.
...
- **Never `React.FC`/`FC`.** Components are function declarations, for example
  `export default function StatCard({ title }: StatCardProps) {}`. (Lint rule bans it.)
